...
function validarLogin($ma, $senha, $pdo) {
    ...
    { // Profissional não encontrado, procurar na tb_alunos
        $sql_check = "SELECT senha FROM tb_alunos WHERE ma = :ma";
        $stmt_check = $pdo->prepare($sql_check);
        $stmt_check->bindParam(':ma', $ma);
        $stmt_check->execute();
        $result = $stmt_check->fetch(PDO::FETCH_ASSOC);

        if ($result && $result['senha'] === $senha_md5){ // Senha corresponde
            // Realizar login
        } else { // Senha inválida
            // Enviar mensagem de erro
        }
...
//Primeiro acesso ao sistema web
function existeDadosVazios($ma, $pdo) {
    $sql_check = "SELECT senha FROM tb_profissionais WHERE ma = :ma";
    $stmt_check = $pdo->prepare($sql_check);
    $stmt_check->bindParam(':ma', $ma);
    $stmt_check->execute();
    ...
    if ($result) { // Profissional encontrado
        if (empty($result['senha'])) { // Senha vazia
            // Redirecionar para cadastro de nova senha
        }
    } else { // Profissional não encontrado, procurar na tb_alunos
        $sql_check = "SELECT senha FROM tb_alunos WHERE ma = :ma";
        $stmt_check = $pdo->prepare($sql_check);
        $stmt_check->bindParam(':ma', $ma);
        $stmt_check->execute();
        ...
        if ($result) { // Aluno encontrado
            ...
            if (empty($result['senha'])) { // Senha vazia
                // Redirecionar para forms de cadastro
            }
        }
        else
        {
            return 0; // Não é o primeiro acesso desse MA, tentar logar
        }
    }
}


function primeiroAcessoProfissional($ma, $senha, $pdo) {
...
//Realiza o cadastro das informações faltantes no banco de dados dos alunos
function primeiroAcessoAluno($ma, $nome, $data_nasc, $cpf, $email, $telefone, $senha, $pdo) {
    $senha_md5 = md5($senha); // Transformando a senha em md5
